...
2. **Ingesta**: RSS Feed Read sobre el feed de Xataka — Inteligencia Artificial, seguido de un Limit a 6 ítems como tope de seguridad (el feed devuelve ~20 items en cada ejecución).
3. **Deduplicación (multi-artículo)**: "Leer Log Completo" lee toda la hoja "Log de Noticias" una vez por ejecución; un Code node ("Filtrar Noticias Nuevas") cruza en bloque los hasta 6 candidatos contra los links ya registrados y deja pasar solo los genuinamente nuevos — soporta procesar varios artículos nuevos en la misma ejecución (p. ej. si se publicó más de uno desde la última vez), no solo el más reciente.
...
- **Deduplicación y log**: Google Sheets, cruzando el log completo contra los candidatos del RSS antes de gastar ninguna llamada de pago — así un artículo ya procesado no vuelve a consumir tokens de Claude ni cuota de Gemini, y el flujo procesa correctamente varios artículos nuevos en la misma ejecución (no solo el más reciente).
